...
def filter_file_paths(file_list):
    """使用函数式编程风格过滤文件名"""
    return list(filter(
        lambda path: len(os.path.basename(path).split('.')) <= 2 
                    and  os.path.splitext(path)[1].lower() == '.edf',
        file_list
    ))
...
if __name__ == '__main__':
    ...
    setup_seed(1)
    file_path_list = iter_files('/root/project/origin_data/TUH_data')
    file_path_list = sorted(file_path_list)
    file_path_list = filter_file_paths(file_path_list)
    random.shuffle(file_path_list)


    path = '/root/project/EEGPT/dataset/pretrain/TUEG_for_pretrain_full'
    os.makedirs(path, exist_ok=True)

    db = lmdb.open(path, map_size=1649267441664)
    file_key_list = []
